    ...
    <div class="">
        <div class="">
            ...
            <div class="w-full flex justify-center items-center">
                <form action="{{route("article.update",$article->id)}}" method="POST" class="w-[80vw] h-[70vh] p-5">
                    <div class="w-full border-2 border-slate-300 p-5 rounded-lg">
                        @csrf
                        @method('put')
                        <div class="flex justify-center items-center">
                            <h1 class="text-2xl text-teal-700 font-bold">Editer l'Article N°{{$article->id}} du titre {{$article->titre}}</h1>
                        </div>
                        <div class="flex justify-center items-center">
                        ...
                        <div class="flex justify-center items-center">
                            <input type="text" class="outline-none border-teal-800 border w-96 h-10 rounded-lg"
                                placeholder="Titre" name="titre" value="{{ $article->titre }}">
                        </div>
                        <div class="flex justify-center">
                            <label for="contenu">Contenu</label>
                        </div>
                        <div class="flex justify-center items-center">
                            <textarea class="outline-none border-teal-800 border w-96 h-20 rounded-lg" name="contenu" cols="30" rows="10">{{ $article->contenu }}</textarea>
                        </div>
                        <br>
                        <div class="flex justify-center items-center">
